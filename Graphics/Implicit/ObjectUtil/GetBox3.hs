...
import Graphics.Implicit.Definitions (ℝ, Fastℕ, Box3, SymbolicObj3 (Rect3R, Sphere, Cylinder, Complement3, UnionR3, IntersectR3, DifferenceR3, Translate3, Scale3, Rotate3, Rotate3V, Shell3, Outset3, EmbedBoxedObj3, ExtrudeR, ExtrudeOnEdgeOf, ExtrudeRM, RotateExtrude, ExtrudeRotateR), SymbolicObj2 (Rotate2, RectR), ExtrudeScale(C1, C2), fromExtrudeScale, (⋯*), fromFastℕtoℝ, fromFastℕ)
        ...
        (twistXmin, twistYmin, twistXmax, twistYmax) =
          let
            ...
            (scalex', scaley') = case scale of
                       C1 s -> (s, s)
                       C2 (sx, sy) -> (sx, sy)
                       f -> both maximum . unzip $ fmap (both abs . fromExtrudeScale f) hrange
                       --Right sfun -> both maximum . unzip $ fmap (both abs . sfun) hrange
            smin s v = min v (s * v)
            smax s v = max v (s * v)
